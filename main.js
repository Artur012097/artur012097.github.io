const config = {
	...
	gemIcons: [
		"images/icons/baozi.svg",
		"images/icons/coffee.svg",
		"images/icons/donut.svg",
		"images/icons/milk.svg",
		"images/icons/pizza.svg",
		"images/icons/strawberry.svg"
	],
...
function createCursor() {
	...
	components.cursor.style.width = config.gemSize - 2 + "px";
	components.cursor.style.height = config.gemSize - 2 + "px";
...
function createGem(t, l, row, col, img) {
	...
	gem.style.borderRadius = `${config.borderRadius}px`;
...
function createGrid() {
	...
	for (i = 0; i < config.countRows; i++) {
		for (j = 0; j < config.countCols; j++) {
			...
			createGem(i * config.gemSize, j * config.gemSize, i, j, config.gemIcons[components.gems[i][j]]);
		}
	}
}
...
function placeNewGems() {
	...
	for (i = 0; i < config.countCols; i++) {
		if (components.gems[0][i] == -1) {
			...
			createGem(0, i * config.gemSize, 0, i, config.gemIcons[components.gems[0][i]]);
			gemsPlaced++;
		}
	}
